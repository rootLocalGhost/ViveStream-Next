...
const AppLogo = () => (
  <svg width="64" height="64" viewBox="0 0 500 500" xmlns="http://www.w3.org/2000/svg" class="setup-logo">
    ...
    <div class="immersive-setup-container">
      <div class="setup-content-card">
        <div class="setup-header">
          <AppLogo />
          <h1 class="setup-title"> viveStream // initialization</h1>
        </div>

        <p class="setup-description">
          ViveStream utilizes the PolyForm Noncommercial License to keep the application footprint minimal and optimized directly for your specific GPU architecture. <br/>
          To enable local playback and hardware transcoding (Intel QSV, NVIDIA NVENC), we must now fetch critical core dependencies (`yt-dlp` and `ffmpeg`) directly from trusted official sources to your local user account. This ensures you always have the most optimized experience.
        </p>
        ...
        <div id="setup-terminal" class="setup-terminal">
          ...
          {loading() && <p class="setup-log-line processing">>> finalization in progress...</p>}
          {logs().length === 0 && <p class="setup-log-line muted">Waiting to begin...</p>}
        </div>
        ...
        >
          {loading() ? (
            <>
              <Loader2 size={24} class="spinIcon muted" /> Processing setup Matrix
            </>
          ) : (
            <>
